...
class QubesVmStorage(object):
    ...
    def reset_volatile_storage(self, verbose = False, source_template = None):
        ...
        if source_template is not None and self.volatile_img:
            ...
            clean_volatile_img = source_template.dir_path + "/clean-volatile.img.tar"
            
            retcode = subprocess.call (["tar", "xf", clean_volatile_img, "-C", self.vm.dir_path])
            if retcode != 0:
               raise IOError ("Error while unpacking %s to %s" % (clean_volatile_img, self.volatile_img))
        # For StandaloneVM create it only if not already exists (eg after backup-restore)
        elif self.volatile_img and not os.path.exists(self.volatile_img):
            if verbose:
