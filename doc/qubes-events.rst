...
asynchronous event handler, add `is_async=True` argument to
...
handler (with `is_async=True`) for synchronous event (the one fired with
...
.. code-block:: python
   ...
   class MyClass(qubes.events.Emitter):
       @qubes.events.handler('event1', 'event2', is_async=True)
       @asyncio.coroutine
       def event_handler(self, event):
           if event == 'event1':
...
.. code-block:: python
   ...
   class MyClass(qubes.events.Emitter):
       @qubes.events.handler('event1', is_async=True)
       @asyncio.coroutine
       def event_handler(self, event):
