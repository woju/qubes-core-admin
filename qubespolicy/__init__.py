...
class PolicyRule(object):
    ...
    def __init__(self, line, filename=None, lineno=None):
        ...
        try:
            ...
            raise PolicySyntaxError('wrong number of fields')

        (action, *params) = self.full_action.split(',')
        try:
            self.action = Action[action]
        except KeyError:
            raise PolicySyntaxError('invalid action: {}'.format(action))
...
class Policy(object):
    ...
    >>> action = policy.eval(system_info, 'source-name', 'target-name')
            ...
            for lineno, line in zip(itertools.count(start=1),
                ...
                if line.startswith('$include:'):
                    ...
                    try:
                        ...
                        raise AccessDenied(
                            'syntax error at {}:{}:{}'.format(path, lineno, e))
    ...
    def eval(self, system_info, source, target):
        ''' Evaluate policy
