...
class TestVM(object):
    def __init__(self):
        self.firewall_conf = '/tmp/test-firewall.xml'
        self.app = TestApp()
...
class TC_06_Expire(qubes.tests.QubesTestCase):
    def test_000_number(self):
        ...
        self.assertEqual(instance.datetime,
            datetime.datetime(2016, 5, 15, 8, 7, 32))
...
class TC_10_Firewall(qubes.tests.QubesTestCase):
    def setUp(self):
        ...
        self.vm = TestVM()
        if os.path.exists(self.vm.firewall_conf):
            os.unlink(self.vm.firewall_conf)
    ...
    def test_002_save_load_rules(self):
        ...
        fw.save()
        self.assertTrue(os.path.exists(self.vm.firewall_conf))
    ...
    def test_003_load_v1(self):
        ...
        """
        with open(self.vm.firewall_conf, 'w') as f:
            f.write(xml_txt)
        with self.assertNotRaises(ValueError):
