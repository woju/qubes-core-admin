...
class Rule(qubes.PropertyHolder):
    ...
    def on_del_proto(self, _event, prop, old_value):
        self.dstports = qubes.property.DEFAULT
        self.icmptype = qubes.property.DEFAULT
...
class Firewall(object):
    ...
    def load(self):
        firewall_conf = self._vm.firewall_conf
        if os.path.exists(firewall_conf):
            ...
            if version == '1':
                ...
                raise qubes.exc.QubesVMError(self._vm,
    ...
    def save(self):
        ...
        try:
            ...
            with open(firewall_conf, 'w') as f:
                xml_tree.write(f, encoding="UTF-8", pretty_print=True)
            os.umask(old_umask)
        except EnvironmentError as err:
            self._vm.log.error("save error: {}".format(err))
            raise qubes.exc.QubesException('save error: {}'.format(err))

        if expiring_rules_present and not self._vm.app.vmm.offline_mode:
            subprocess.call(["sudo", "systemctl", "start",
                             "qubes-reload-firewall@%s.timer" % self._vm.name])
